<x-app-layout title="Dashboard Admin">
    ...
    <div class="card my-4">
        <div class="card-header" style="background: #e9500e; color: #ffffff;">
            List Summary Department "{{ $department }}"
        </div>
        <div class="card-body">
            <x-alert />
                    ...
                    <table class="table table-bordered" id="dataTable">
                        ...
                        <tbody>
                            @php
                                ...
                                $grandTotal = 0;
                            @endphp
                            @foreach ($totalResponsesPerCategory as $category => $totalResponses)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td><a
                                            href="{{ route('admin.summary.response.category', $category) }}">{{ $category }}</a>
                                    </td>
