<x-app-layout title="Dashboard Admin">
    ...
    <div class="card my-4">
        <div class="card-header" style="background: #e9500e; color: #ffffff;">
            List Response Without Department "{{ $departments }}"
        </div>
        <div class="card-body">
            <x-alert />
                        ...
                        <tbody>
                            ...
                            @foreach ($employees as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->question }}</td>
                                    <td>{{ $item->answer }}</td>
                                    <td>{{ optional($item->created_at)->format('d M Y, H:i:sa') ?? 'NA' }}</td>
                                    <td>{{ optional($item->updated_at)->format('d M Y, H:i:sa') ?? 'NA' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
